...
async function run() {
    ...
    const allDataCollection = client.db('data-dashboard').collection('data')
    const cartsCollection = client.db('data-dashboard').collection('carts')
    const reviewsCollection = client.db('data-dashboard').collection('reviews')
    const usersCollection = client.db('data-dashboard').collection('users')
    const myprofileCollection = client.db('data-dashboard').collection('myprofiles')
    const paymentCollection = client.db('data-dashboard').collection('payments')
...
app.get("/alldata", async (req,res)=>{
    ...
    foods = await allDataCollection.find().toArray()
    res.send(foods)
})
...
app.post("/adddomain",verifyJWT,verifyAdmin, async (req,res)=>{
     ...
     const food = req.body
    domains = await allDataCollection.insertOne(food)
...
app.delete("/deletedomain/:id",verifyJWT, async (req,res)=>{
 ...
 deleteItem = await allDataCollection.deleteOne(filter)
...
app.put("/editdomain/:id",verifyJWT, async (req,res) =>{
  ...
  const id = req.params.id
    ...
    const editItem = await allDataCollection.updateOne(filter,updateDoc,options)
    res.send(editItem)
})
